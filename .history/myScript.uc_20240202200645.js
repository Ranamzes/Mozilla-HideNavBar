		...
		try {
			...
			let mouseMoveHandler = function (e) {
				try {
					...
					let currentWindow = e.view || window.content;
					let currentURL = contentWindow.location.href;

					// Обновляем элемент лога
					let logElement = contentWindow.document.getElementById("navBarModifierLog");
					if (logElement) {
							logElement.textContent = `Current URL: ${currentURL}`;
							if (currentURL.includes("addons.mozilla.org") || currentURL.includes("metrika.yandex.ru")) {
									logElement.textContent += " | On AMO or Metrika - Keeping navBar visible";
